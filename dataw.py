# -*- coding: utf-8 -*-
"""
Created on Mon Jun 19 17:48:38 2017
...
def processTicket():
    ...
    df['TicketPrefixId'] = pd.factorize(df['TicketPrefix'])[0]  #它将一个数组转换成两个数组：labels和levels。其中levels包含原数组中的所有不重复的值，
                                                                #labels则是一个长度和原始数组相同，值为从0到len(levels)-1的整数数组，
                                                                #其中的每个值表示原始数组中对应值在levels中的下标。
    if keep_binary:
        cletters = pd.get_dummies(df['TicketPrefix'].rename(columns=lambda x: 'TicketPrefix_' + str(x)))
        df = pd.concat([df, cletters], axis=1)
...
def getTicketPrefix(ticket):                                                   #用正则表达式提取字母
...
def processEmbarked():
    global df
    df.Embarked[ df.Embarked.isnull() ] = df.Embarked.dropna().mode().values   #缺失值用众数填充 mode（）【众数】
    df['Embarked'] = pd.factorize(df['Embarked'])[0]                           #它将一个数组转换成两个数组：labels和levels。其中levels包含原数组中的所有不重复的值，
                                                                               #labels则是一个长度和原始数组相同，值为从0到len(levels)-1的整数数组，
                                                                               #其中的每个值表示原始数组中对应值在levels中的下标。
    if keep_binary:
...
def processPClass():
    global df
    df.Pclass[ df.Pclass.isnull() ] = df.Pclass.dropna().mode().values         #缺失值用众数填充 mode（）【众数】
    if keep_binary:
...
def processFamily():
    global df
    df['SibSp'] = df['SibSp'] + 1                                              #1代表自己
    df['Parch'] = df['Parch'] + 1                                              #1代表自己
    if keep_scaled:
        scaler = preprocessing.StandardScaler()
...
def familysize():
    ...
    if keep_binary:
        df = pd.concat([df, pd.get_dummies(df['Singleton']).rename(columns=lambda x: 'Singleton' + str(x))], axis=1)
        df = pd.concat([df, pd.get_dummies(df['SmallFamily']).rename(columns=lambda x: 'SmallFamily' + str(x))], axis=1)
        df = pd.concat([df, pd.get_dummies(df['LargeFamily']).rename(columns=lambda x: 'LargeFamily' + str(x))], axis=1)
...
def processName():
    global df
    df["Names"] = df["Name"].map(lambda x:len(re.split(" ",x)))               #按照空格拆分，提取名字字符数
    df['Title'] = df["Name"].map(lambda x:re.compile(",(.*?)\.").findall(x)[0]) #提取称呼
    
    df['Title'][df.Title == 'Jonkheer'] = 'Master'                             #Jonkheer 尊称
    df['Title'][df.Title.isin(['Ms','Mlle'])] = 'Miss'                         #'Ms','Mlle'  女士称呼
    df['Title'][df.Title == 'Mme'] = 'Mrs'                                     #'Mme'  男士称呼
    df['Title'][df.Title.isin(['Capt', 'Don', 'Major', 'Col', 'Sir'])] = 'Sir' #'Capt', 'Don', 'Major', 'Col', 'Sir' 合并到 sir
    df['Title'][df.Title.isin(['Dona', 'Lady', 'the Countess'])] = 'Lady'      #'Dona', 'Lady', 'the Countess'合并到 Lady
...
def getDataSets(binary=False, bins=False, scaled=False, strings=False, \
    ...
    df = pd.concat([input_df,submit_df])                                       #合并数据
    df.reset_index(inplace=True)                                               #重设索引
    df.drop("index",axis=1,inplace=True)                                      #删除原有索引值
    df = df.reindex_axis(input_df.columns,axis=1)                              #列名重设
    processCabin()
    processTicket()
    processName()
    processFare()    
...
'''    
    ...
    numerics = df.loc[:, ['Age_scaled', 'Fare_scaled', 'Pclass_scaled', 'Parch_scaled', 'SibSp_scaled', 
                          'Names_scaled', 'CabinNumber_scaled', 'Age_bin_id_scaled', 'Fare_bin_id_scaled']]                   #数据中进行标准化的特征
    ...
    for i in range(0, numerics.columns.size-1):                                
        for j in range(0, numerics.columns.size-1):
            if i <= j:
                name = str(numerics.columns.values[i]) + "*" + str(numerics.columns.values[j])                      #特征与特征积
                df = pd.concat([df, pd.Series(numerics.iloc[:,i] * numerics.iloc[:,j], name=name)], axis=1)
                new_fields_count += 1
            if i < j:
                name = str(numerics.columns.values[i]) + "+" + str(numerics.columns.values[j])                      #特征与特征和
                df = pd.concat([df, pd.Series(numerics.iloc[:,i] + numerics.iloc[:,j], name=name)], axis=1)
                new_fields_count += 1
            if not i == j:
                name = str(numerics.columns.values[i]) + "/" + str(numerics.columns.values[j])                      #特征与特征除
                df = pd.concat([df, pd.Series(numerics.iloc[:,i] / numerics.iloc[:,j], name=name)], axis=1)
                name = str(numerics.columns.values[i]) + "-" + str(numerics.columns.values[j])                       #特征与特征减
                df = pd.concat([df, pd.Series(numerics.iloc[:,i] - numerics.iloc[:,j], name=name)], axis=1)
                new_fields_count += 2
